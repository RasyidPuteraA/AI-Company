...
REPORT_DIR="company/reports/weekly"
mkdir -p "$REPORT_DIR"

OUT="$REPORT_DIR/$(date +%F)-weekly-report.md"
SNAP="/tmp/ai-company-weekly-report-$(date +%F).json"

python3 - "$OUT" "$SNAP" "7" "AI Company OS Weekly Review Report" <<'INNERPY'
import json
import subprocess
import sys
