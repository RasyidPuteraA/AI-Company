...
case "$ROLE:$MODE" in
  pm:client)
    task_key="$(claim_task client)"
    if [ -z "$task_key" ]; then
      write_role_state "idle" "" "No client PM task"
      exit 0
    fi
  ...
  engineer:client)
    task_key="$(claim_task client)"
    if [ -z "$task_key" ]; then
      write_role_state "idle" "" "No client engineer task"
      exit 0
    fi
    project_key="$(task_project "$task_key")"
  ...
  qa:client)
    task_key="$(claim_task client)"
    if [ -z "$task_key" ]; then
      write_role_state "idle" "" "No client QA task"
      exit 0
    fi
    project_key="$(task_project "$task_key")"
  ...
  devops:client)
    task_key="$(claim_task client)"
    if [ -z "$task_key" ]; then
      write_role_state "idle" "" "No client DevOps task"
      exit 0
    fi
